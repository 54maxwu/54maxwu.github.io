...
export async function disruptWatermark(canvas, options = {}) {
    ...
    for (const t of techniques) {
        ...
        try {
            if (t === 'geom')       { const r = geom(canvas, intensity);       log.push(`幾何擾動: crop ${(r.crop*100).toFixed(2)}% (${ms(t0)})`); }
            else if (t === 'noise') { const r = noise(canvas, intensity);      log.push(`高斯噪聲: σ≈${r.sigma.toFixed(1)} (${ms(t0)})`); }
            else if (t === 'unsharp'){ unsharp(canvas, 0.5);                   log.push(`銳化補償 (${ms(t0)})`); }
            else if (t === 'doubleJpeg') {
                const midQ = Math.max(0.55, 0.75 - 0.03 * intensity);
                const r = await doubleJpeg(canvas, midQ);
                log.push(r.skipped ? `雙次JPEG: 跳過 (toBlob 失敗)` : `雙次JPEG: 中間 q=${Math.round(r.midQ*100)} (${ms(t0)})`);
            }
            else if (t === 'chShift') {
                ...
                log.push(`低頻帶狀噪聲: ${r.bandScale} amp=${r.amp.toFixed(1)} (${ms(t0)})`);
            }
            else if (t === 'fftPhase') {
                const maxDev = Math.PI / 60 * (intensity / 3);
                const r = await fftPhase(canvas, maxDev);
                log.push(`FFT 相位擾動: ${r.N}² @ ±${r.maxDevDeg}° (${ms(t0)})`);
            }
            else if (t === 'median') {
                if (intensity >= 3) { median3(canvas); log.push(`中值濾波 3×3 (${ms(t0)})`); }
                else { log.push(`中值濾波: 跳過 (intensity<3)`); continue; }
            }
            else continue;
            applied.push(t);
        } catch (err) {
            log.push(`${t} 失敗: ${err.message}`);
        }
    }
