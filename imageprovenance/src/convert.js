...
//   3. Inject fake camera EXIF via piexifjs (bundled locally).

import piexif from 'piexifjs';
async function loadPiexif() { return piexif; }
...
export async function convertImage(inputBytes, inputMime, profile, opts = {}) {
    ...
    log.push(`解碼成功: ${canvas.width}×${canvas.height}, 重編碼為 JPEG q=${Math.round(quality*100)}`);
    ...
    if (opts.advanced?.dateTime) log.push(`  · 拍攝時間: ${new Date(opts.advanced.dateTime).toLocaleString('zh-CN')}`);
