...
const STRINGS = {
    ...
    'model.disclaimer':     { zh: '瀏覽器內運行的神經網路 · 首次需下載權重 (~25–45MB) · 圖片不上傳,僅下載模型',  en: 'A neural net running in your browser · first run downloads ~25–45MB of weights · the image is never uploaded, only the model is downloaded' },
    ...
    'model.resultDisclaimer':{ zh: '模型 ${id} · 後端 ${dev} · 小型偵測器對壓縮圖/手機照/翻拍可能誤報為 AI,綜合判定已對此票降權並設上限,僅作一票參考。', en: 'Model ${id} · backend ${dev} · small detectors can misfire (flag real/compressed/phone photos as AI); the combined verdict down-weights and caps this vote — treat it as one reference vote.' },
