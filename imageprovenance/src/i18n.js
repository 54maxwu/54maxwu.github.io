...
const STRINGS = {
    ...
    'fusion.detail.modelNeutral':{ zh: '模型 ${pct}% · 信心不足,不計入',                                en: 'model ${pct}% · low confidence, not counted' },
    ...
    'model.disclaimer':     { zh: '瀏覽器內運行的 CLIP 零樣本判斷 · 首次需下載權重 (~85MB) · 圖片不上傳,僅下載模型',  en: 'CLIP zero-shot, running in your browser · first run downloads ~85MB of weights · the image is never uploaded, only the model is downloaded' },
    ...
    'model.resultDisclaimer':{ zh: '${id} · 後端 ${dev} · CLIP 零樣本:用語意比對「真實 vs AI」,比小型分類器泛化好,但仍非精準偵測器。只有 >80% 或 <30% 才有參考性,中間區間綜合判定不計入。', en: '${id} · backend ${dev} · CLIP zero-shot: semantic "real vs AI" comparison — generalizes better than tiny classifiers but is still not a precise detector. Only >80% or <30% is meaningful; the mid-range is ignored by the combined verdict.' },
    ...
    'forensics.ela.hint':   { zh: '重壓縮後的差異 · 一塊區域明顯比周圍亮 = 可能被貼上/重繪。邊緣本來就會亮,看的是「區塊」而非線條。', en: 'Recompression difference · a patch much brighter than its surroundings may be pasted/inpainted. Edges always glow — look for blobs, not lines.' },
    ...
    'forensics.noise.hint': { zh: '逐區塊的噪聲地板 · 綠=均勻(正常),紅=異常。真實照片噪聲均勻;某區異常平滑/粗糙 = 可能拼接或重繪。', en: 'Per-block noise floor · green = uniform (normal), red = anomalous. A genuine photo has even noise; an abnormally smooth/rough region may be spliced or inpainted.' },
    'forensics.noise.stat': { zh: '不均係數 ${cov} · 連片異常 ${contig} 塊',                            en: 'CoV ${cov} · ${contig} contiguous anomalous blocks' },
    'forensics.ghost.title':{ zh: '③ JPEG Ghost',                                                      en: '③ JPEG Ghost' },
    'forensics.ghost.hint': { zh: '掃描多個壓縮品質 · 紅色區塊的壓縮歷史與全圖不同 = 可能來自另一張圖貼上。', en: 'Sweeps many compression qualities · red blocks have a different compression history from the rest = possibly pasted from another image.' },
    'forensics.ghost.stat': { zh: '主壓縮品質 q≈${q} · 異常連片 ${dev} 塊',                             en: 'dominant quality q≈${q} · ${dev} contiguous deviating blocks' },
    'forensics.clone.title':{ zh: '④ 複製-貼上偵測',                                                    en: '④ Copy-move detection' },
    'forensics.clone.hint': { zh: '找出同圖內被複製的區塊(用來遮蓋或重複內容)· 青色連線標出共享同一位移的克隆對。', en: 'Finds blocks duplicated within the image (used to cover or repeat content) · cyan lines mark clone pairs sharing one translation.' },
