...
- **AI 模型偵測(真模型)** — transformers.js 在瀏覽器內跑 `onnx-community/ai-source-detector-ONNX`(ViT,含 `real` 類別與 SD / Midjourney / DALL·E / other 各生成器分類),WebGPU 加速、失敗退回 WASM。輸出校準 AI 機率 + 廠商拆分。**圖片不上傳,只下載模型權重**(首次 ~25–45MB,之後瀏覽器快取)
...
- **AI 模型**(`ai-source-detector`,ViT,含 `real` 類別與各生成器分類)是真正訓練過的分類器,比二分類 detector 更會泛化;但小型模型仍可能對壓縮圖/手機照/翻拍**誤報為 AI**,所以綜合判定對它**降權 + 設上限**,只當一票。首次需連網下載權重 (~25–45MB)。
